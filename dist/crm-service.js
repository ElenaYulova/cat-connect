...
module.exports = cds.service.impl(async function () {
    ...
    */
    this.before(['CREATE', 'UPDATE'], [Customers, Customers.drafts], async (req) => {
        ...
        // 3. Notes in Drafts
        const draftNotes = await cds.run(SELECT.from(CustomerNotes.drafts)
    ...
    */
    ...
    this.after('READ', 'Customers', async (each) => {
        ...
        for (const customer of customers) {
            ...
            const boughtCategories = await cds.run(SELECT.from(OrderItems)
            ...
            const categoryObj = await cds.run(SELECT.one.from(UserCategories).columns('name').where({ code: favoriteCode }));
