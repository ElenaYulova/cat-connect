...
class SalesOrderService extends cds_1.default.ApplicationService {
    async init() {
        ...
        */
        this.before(['CREATE', 'UPDATE'], 'Orders', async (req) => {
            ...
            for (const item of order.items) {
                if (!item.game_id)
                    continue;
                const game = await cds_1.default.db.run(SELECT.one.from(Products).where({ id: item.game_id }));
                if (game) {
            ...
            if (order.customer_id) {
                const gamerProfile = await cds_1.default.db.run(SELECT.one.from('SalesOrderService.CustomerInsights')
        ...
        */
        this.before('UPDATE', 'Orders', async (req) => {
            ...
            if (!currentOrder)
                return;
            if (currentOrder.status_code !== 'P' && currentOrder.status_code !== 'C')
                return;
            // Check prev state
            const previousState = await cds_1.default.db.run(SELECT.one.from(Orders).where({ id: currentOrder.id }).columns('status_code'));
            if (previousState && (previousState.status_code === 'P' || previousState.previousState === 'C')) {
                return;
            }
            // Draft tables support
            const draftOrder = await cds_1.default.db.run(SELECT.one.from(req.target.name)
            ...
            for (const item of draftOrder.items) {
                if (!item.game_id)
                    continue;
                const product = await cds_1.default.db.run(SELECT.one.from(Products).where({ id: item.game_id }));
                if (!product)
                    continue;
                ...
                await cds_1.default.db.run(cds_1.default.update(Products)
        ...
        */
        this.on('READ', 'Orders', async (req, next) => {
            ...
            if (queryStr.includes('VIP_FILTER_ACTIVE')) {
                const vipOrders = await cds_1.default.db.run(SELECT.from(Orders).where({
        ...
        */
        this.on('checkBulkEligibility', 'Orders', async (req) => {
            const { id: orderId } = req.params;
            const { qty: bulkThreshold } = req.data;
            if (!orderId || !bulkThreshold)
                return false;
            const items = await cds_1.default.db.run(SELECT.from('SalesOrderService.OrderItems').where({ parent_id: orderId }).columns('quantity'));
