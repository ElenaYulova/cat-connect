...
module.exports = cds.service.impl(async function () {
    ...
    */
    this.after(['CREATE', 'UPDATE'], 'Feedbacks', async (data, req) => {
        ...
        const lastOrder = await SELECT.one.from(Orders).columns('createdAt').where({ customer_ID }).orderBy('createdAt desc');
        let refDate = lastOrder?.createdAt ? new Date(lastOrder.createdAt) : new Date();

        if ((new Date() - refDate) / (1000 * 3600 * 24) > 180) {
            await UPDATE(Customers, customer_ID).with({ statusCode_code: 'I' });
            return;
        }


        const result = await SELECT.one.from(Feedbacks).columns('avg(rating) as avgRating').where({ customer_ID });
        const avg = result.avgRating ? parseFloat(result.avgRating.toFixed(2)) : 0;

        const currentRating = data.rating ?? req.data?.rating;
        const status = (currentRating < 3 || (avg > 0 && avg < 3)) ? 'R' : 'A';
